...
class BBD_Field {
	...
	public function get_html( $post_id = 0 ) {
		...
		if( 'image' == $this->type && ! empty( $value ) ){
			...
			$link = '';
			$image_key = get_field( $this->key, get_the_ID() );
			$image_alt = $image_key['alt'];
			...
			<div class="<?php echo $wrapper_class; ?>" >
				...
				<?php } ?>
						<?php if( $image_alt ) { ?>
							<img src="<?php echo esc_url( $value ); ?>" alt="<?= $image_alt ?>" />
						<?php } else { ?>
							<img src="<?php echo esc_url( $value ); ?>" alt="<?= get_the_title() ?>" />
						<?php } ?>
				<?php if( $link ) { ?>
					</a>
				<?php } ?>
		...
		if( 'gallery' == $this->type ) {
			...
			<?php
				...
				?>
					...
					>
						<?php if( $gallery_alt ) { ?>
							<img class="bbd-image <?php echo $this->key . " " . $image_num; ?>" src="<?php echo $thumbnail_url; ?>" alt="<?= $gallery_alt ?>" />
						<?php } else { ?>
							<img class="bbd-image <?php echo $this->key . " " . $image_num; ?>" src="<?php echo $thumbnail_url; ?>" alt="<?= get_the_title() ?> Gallery Image <?= $image_num ?>" />
						<?php } ?>
					</a>
				<?php
